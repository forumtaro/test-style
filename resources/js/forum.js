...
app.initializers.add('forum-taro-theme', () => {
  ...
  (function () {
    ...
    if (els.mpc) {
      els.mpc.addEventListener('click', function (e) {
        const link = e.target.closest('a[data-route]');
        if (link) { e.preventDefault(); goTo(link.dataset.route); }
      });
    }
    ...
    function updateNav() {
      ...
      if (u) {
        ...
        if (els.bookmarks) {
          ...
          if (els.bookmarksText) els.bookmarksText.textContent = 'Закладки';
        ...
        if (els.privateText) els.privateText.textContent = 'Про нас';
        ...
        if (els.bookmarks) {
          ...
          if (els.bookmarksText) els.bookmarksText.textContent = 'Значення';
    ...
    function initNav() {
      ...
      if (app.session && app.session.user) {
        ...
        if (els.mpc) els.mpc.style.display = 'none';
        if (els.loginBtn) {
          els.loginBtn.style.display = 'flex';
          els.loginBtn.onclick = () => {
            app.modal.show(flarum.core.compat['forum/components/LogInModal']);
          };
        }
      }
      updateNav();
    ...
    const t = setInterval(() => {
      if (app.session) { clearInterval(t); initNav(); }
    }, 300);
  ...
  (function () {
    ...
    function createNotificationIcon() {
      ...
      if (loggedIn) {
        ...
        notificationIcon.addEventListener('click', function (e) {
          e.preventDefault();
          e.stopPropagation();
          if (window.m && m.route) m.route.set('/notifications');
        ...
        notificationIcon.addEventListener('click', function (e) {
          e.preventDefault();
          e.stopPropagation();
          if (window.m && m.route) m.route.set('/info');
    ...
    function removeNotificationIcon() {
      if (notificationIcon && notificationIcon.parentNode) notificationIcon.remove();
    ...
    function updateNotificationCount() {
      ...
      try {
        if (app?.session?.user) {
          const count = app.session.user.unreadNotificationCount();
          updateBadge(count);
        }
      } catch (e) {
        setTimeout(() => {
          try {
            if (notificationCount && app?.session?.user) {
              const count = app.session.user.unreadNotificationCount();
              updateBadge(count);
            }
          } catch (e) {}
    ...
    function setupFlarumIntegration() {
      ...
      if (app.session) {
        app.session.on && app.session.on('login', () => setTimeout(forceUpdate, 200));
        app.session.on && app.session.on('logout', () => setTimeout(forceUpdate, 200));
      }
    }
    ...
    function initialCheck() {
      ...
      const interval = setInterval(() => {
        forceUpdate();
        checks++;
        if (checks >= 10 || app?.session) {
          clearInterval(interval);
          if (app?.session) setupFlarumIntegration();
        }
      }, 500);
    }
    ...
    window.addEventListener('popstate', () => setTimeout(forceUpdate, 200));
    window.addEventListener('focus', () => {
      if (isUserLoggedIn()) updateNotificationCount();
      else forceUpdate();
    }, { passive: true });

    const observer = new MutationObserver(() => {
      if (!notificationIcon) forceUpdate();
    });
  ...
  (function () {
    function createLogo() {
      ...
      logoContainer.style.cssText = 'position:fixed;top:0;left:50%;transform:translateX(-50%);z-index:1001;transition:opacity 0.3s, transform 0.3s;';
      ...
      img.alt = 'Логотип';
      img.style.cssText = 'max-width:170px;height:auto;display:block;';
    ...
    function handleScroll() {
      ...
      const scrubber = document.querySelector('.App-titleControl') || document.querySelector('.PostStreamScrubber');
  ...
  (function () {
    ...
    function setupDropdowns() {
      document.querySelectorAll('.Dropdown-menu').forEach(dd => {
        ...
        dd.addEventListener('touchmove', e => {
          ...
          if (delta > 0 && canClose) { e.preventDefault(); applyDrag(dd, delta); }
        }, { passive: false });
    ...
    function startSwipe() {
      if (!document.body) { setTimeout(startSwipe, 50); return; }
      setupDropdowns();
      observer.observe(document.body, { childList: true, subtree: true });
    }
  ...
  (function () {
    ...
    style.textContent = `
      blockquote.collapsible { max-height: 80px; overflow: hidden; position: relative; }
      blockquote.collapsible.expanded { max-height: none; padding-bottom: 35px; }
      blockquote.collapsible::after { content: ''; position: absolute; bottom: 0; left: 22px; right: 0; height: 40px; background: linear-gradient(transparent, var(--code-bg)); }
      blockquote.collapsible.expanded::after { display: none; }
      .quote-toggle { position: absolute; bottom: 6px; right: 8px; z-index: 2; padding: 2px 10px; background: white; color: #6c7d93; border: 1px solid #6c7d93; border-radius: 3px; cursor: pointer; font-size: 12px; }
      .quote-toggle:hover { background: #6c7d93; color: white; }
    `;
